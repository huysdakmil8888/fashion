...
class SubscribeModel extends AdminModel
{
        ...
        protected $folderUpload        = 'subscribe' ;
        protected $fieldSearchAccepted = ['id', 'name', 'description', 'link'];
        protected $crudNotAccepted     = ['_token','thumb_current','_method','url'];
    ...
    public function listItems($params = null, $options = null) {
        ...
        if($options['task'] == 'news-list-items') {
            $query = self::select('email', 'status')
                ->where('status', 'active' );
    ...
    public function getItem($params = null, $options = null) { 
        ...
        if($options['task'] == 'get-email') {
            $result = self::select('email')->where('email', $params['email'])->first();
        }
        if($options['task'] == 'change-status') {
            $result = self::where('email', $params['email'])->update(["status"=>"inactive"]);
        }
